...
window.Square_2 = window.classes.Square_2 =
    class Square_2 extends Shape              // A square, demonstrating two triangles that share vertices.  On any planar surface, the interior
                                            // edges don't make any important seams.  In these cases there's no reason not to re-use data of
    {                                       // the common vertices between triangles.  This makes all the vertex arrays (position, normals,
      constructor()                         // etc) smaller and more cache friendly.
      { super( "positions", "normals", "texture_coords" );                                   // Name the values we'll define per each vertex.
...
window.Cube_2 = window.classes.Cube_2 =
    class Cube_2 extends Shape    // A cube inserts six square strips into its arrays.
    { constructor()
    { super( "positions", "normals", "texture_coords" );
      for( var i = 0; i < 3; i++ )
        for( var j = 0; j < 2; j++ )
        ...
        { if( GOURAUD || COLOR_NORMALS )    // Do smooth "Phong" shading unless options like "Gouraud mode" are wanted instead.
          { gl_FragColor = VERTEX_COLOR;    // Otherwise, we already have final colors to smear (interpolate) across vertices.            
            return;
                                            ...
                                            // Phong shading is not to be confused with the Phong Reflection Model.
          vec4 to4 = vec4(f_tex_coord,0.0,0.0);
          mat4 translate = mat4(vec4(1.0,0.0,0.0,0.0),vec4(0.0,1.0,0.0,0.0),vec4(0.0,0.0,1.0,0.0),vec4(mod(2.0*animation_time, 50.0), 0.0, 0.0, 1.0));
          to4 += vec4(1.0,1.0,0.0,1.0);
          to4 = translate*to4;
          
          vec4 tex_color = texture2D( texture, to4.xy);                         // Sample the texture image in the correct place.
                                                                                      // Compute an initial (ambient) color:
          if( USE_TEXTURE ) gl_FragColor = vec4( ( tex_color.xyz + shapeColor.xyz ) * ambient, shapeColor.w * tex_color.w ); 
          else gl_FragColor = vec4( shapeColor.xyz * ambient, shapeColor.w );
          gl_FragColor.xyz += phong_model_lights( N );                     // Compute the final color with contributions from lights.
        ...
        { if( GOURAUD || COLOR_NORMALS )    // Do smooth "Phong" shading unless options like "Gouraud mode" are wanted instead.
          { gl_FragColor = VERTEX_COLOR;    // Otherwise, we already have final colors to smear (interpolate) across vertices.            
            return;
                                            ...
                                            // Phong shading is not to be confused with the Phong Reflection Model.
          vec4 to4 = vec4(f_tex_coord,0.0,0.0);
          mat4 translate = mat4(vec4(cos(mod(.5*3.14*animation_time, 50.0)),sin(mod(.5*3.14*animation_time, 50.0)),0.0,0.0),vec4(-sin(mod(.5*3.14*animation_time, 50.0)),cos(mod(.5*3.14*animation_time, 50.0)),0.0,0.0),vec4(0.0,0.0,1.0,0.0),vec4(0.0, 0.0, 0.0, 1.0));
          to4 -= vec4(.5,.5,0.0,0.0);
          to4 = translate*to4;
          to4 += vec4(.5,.5,0.0,0.0);
          vec4 tex_color = texture2D( texture, to4.xy );                         // Sample the texture image in the correct place.
                                                                                      // Compute an initial (ambient) color:
